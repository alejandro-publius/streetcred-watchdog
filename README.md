...
| Tests | 630, offline, no credentials, under a second |
...
`google-auth` beneath it), all 630 tests green with no network and no credentials.
...
pytest -q                        # 630 tests, no network, no credentials
...
| `tests/` | 630 of them. The cases where a naive implementation produces a confident lie. |
