...
| Tests | 608, offline, no credentials, under a second |
...
`google-auth` beneath it), all 608 tests green with no network and no credentials.
...
pytest -q                        # 608 tests, no network, no credentials
...
| `tests/` | 608 of them. The cases where a naive implementation produces a confident lie. |
