...
| Tests | 605, offline, no credentials, under a second |
...
`google-auth` beneath it), all 605 tests green with no network and no credentials.
...
pytest -q                        # 605 tests, no network, no credentials
...
| `tests/` | 605 of them. The cases where a naive implementation produces a confident lie. |
